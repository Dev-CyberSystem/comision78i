...
<<<<<<< HEAD
const listaFiltro = document.getElementById("listaProductosFiltrados")
=======
const listaFiltrada = document.getElementById("listaFiltrada");
>>>>>>> a843d005cbdeafa2c32d9e1b8a7e74f74d33c4bb
...
buttonFiltrarProducto.addEventListener("click", (e) => {
  ...
  if(valorFiltrado.length === 0){
    ...
    valorFiltrado.forEach((producto, index) => {
      const li = document.createElement("li");
      li.textContent = `${index + 1} - ${producto}`;
      listaFiltro.appendChild(li);
    }) //Repito un proceso similar al listado de productos, como en "valorFiltrado" se guarda el array con los diferentes productos coincidentes, realizo un forEach de ese array para que cree un li de acuerdo a la cantidad de datos que contenga el array :)
  ...
  if (valorFiltrado.length === 0) {
    ...
    valorFiltrado.forEach((producto, index) => {
      ...
      botonEliminar.addEventListener("click", () => {
        ...
        listaFiltrada.removeChild(li);
        resultadoFiltrado.textContent = `Se elimino el producto del carrito ${producto}`;
